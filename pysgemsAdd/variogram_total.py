...
def read_col(fname, col, convert=str, sep='\t'):
    """ Function to get the data of one specific column. """
    with open(fname) as fobj:
        return [convert(line.split(sep=sep)[col]) for line in fobj]


def variogram(coords, values, nr_lags, weights=None, max_dist=None, lag_type='equalSize', lag_edges=None, poisson=False, anisotropy=False, theta_step=90, var_type='gamma', plotit=True, subsample=None):
        """
...
def variogramfit(inputfile, distances, values, vg_model, nr_pairs=None, vg_range=[], vg_sill=[], vg_nugget=[], weightfun='none', plotit=True, full=True):
        """
        ...
        Parameters
        ----------
        inputfile : used for the filename of the xml file
...
def main(inputfile, nbpoints):
    start = time.time()
    # get the coordinates and the values of the parameter
    columnx = 0
    columny = 1
    columnAs = 2
    columnx = int(columnx)
    ...
    variogram(coords, values, nr_lags=nbpoints)  # creating the points on the variogram
    ...
    variogramfit(inputfile, distances, values, model, nr_pairs, vg_range, vg_sill, vg_nugget)  # fitting the best curve to the variogram plot and creating the xml file for kriging
